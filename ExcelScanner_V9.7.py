...
class UltimateSecuritySuite:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel Intel & Security v9.7")
    ...
    def pre_launch_cleanup(self):
        """Clean RAM and kill stuck Excel processes before starting."""
        try:
    ...
    def perform_deep_scan(self):
        try:
            ...
            # 2. FILE SCAN (SECURITY & PERFORMANCE)
            ...
            if wb.HasVBProject:
                problems.append({"issue": "VBA Macros Detected", "reason": "Macros can contain viruses or harmful automation scripts.", "sol": "Inspect code or remove macros if unknown.", "id": "VBA"})
            
            if ext_links:
                problems.append({"issue": "External Connections", "reason": "Links to other files can leak data or cause 'Security Warnings'.", "sol": "Break links to stabilize the file.", "id": "LINKS"})

            # Performance Check
            if load_time > 4.0:
                problems.append({"issue": "High Loading Time", "reason": f"File took {load_time:.2f}s to open (Threshold is 4s).", "sol": "Clean metadata and convert to Binary format.", "id": "LOAD"})
            ...
            for sh in wb.Sheets:
                ...
                if used_r > data_c + 3000:
                    problems.append({"issue": f"Row Bloat: {sh.Name}", "reason": f"Sheet uses {used_r} rows but only has {data_c} data cells.", "sol": "Delete ghost rows to reduce file size.", "id": "GHOST"})
    ...
    def display_report(self):
        ...
        # FIXED BUG: Using float() instead of int() to handle '91.8'
        ...
        # --- SECTION: PROBLEM & SOLUTION CARDS ---
        tk.Label(self.container, text="Found Problems & Recommendations", font=("Segoe UI", 12, "bold"), bg=CLR_BG).pack(anchor="w")
        
        if not self.audit_data['problems']:
            tk.Label(self.container, text="✅ File and System are optimized.", font=("Segoe UI", 10), fg=CLR_EXCEL, bg=CLR_BG).pack(anchor="w", pady=10)
    ...
    def run_fix(self, task_id):
        # [Repair Logic for All Tasks]
        messagebox.showinfo("Repair Engine", f"Processing {task_id} Optimization...")
        self.pre_launch_cleanup() # Re-flush RAM during fix
