...
class ExcelProOptimizer:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel Master Optimizer v7.0")
    ...
    def show_home(self):
        self.clear_ui()
        tk.Label(self.container, text="Excel Master Optimizer", font=("Segoe UI", 28, "bold"), bg=CLR_BG, fg=CLR_PRI).pack(pady=(80, 10))
        tk.Label(self.container, text="System Hardware & File Logic Diagnostic Suite", font=("Segoe UI", 12), bg=CLR_BG, fg="#605E5C").pack(pady=5)
    ...
    def run_full_audit(self):
        try:
            ...
            self.log("Analyzing Row/Column density and external links...")
            sheet_data = []
            issues = []
            for sh in wb.Sheets:
                ...
                if r > (data + 1500): issues.append(("Ghost Rows", sh.Name, "Cleanup Ghost Rows"))

            if wb.HasVBProject: issues.append(("VBA Detected", "Project", "Verify Macros"))
    ...
    def show_results(self):
        ...
        tk.Label(header, text=f"File: {os.path.basename(self.file_path)} | Size: {self.results['size']:.2f} MB", bg=CLR_BG).pack(side="right")

        # Table
        tbl_frame = tk.Frame(self.container)
        tbl_frame.pack(fill="x", pady=10)
        cols = ("Sheet Name", "Used Rows", "Used Cols", "Real Data Cells")
        ...
        # Repair Hub
        hub = tk.LabelFrame(self.container, text=" Optimization Tools ", font=("Segoe UI", 11, "bold"), bg=CLR_BG, padx=20, pady=20)
        hub.pack(fill="x", pady=10)

        for issue, loc, action in self.results['issues']:
            f = tk.Frame(hub, bg=CLR_BG)
            f.pack(fill="x", pady=3)
            tk.Label(f, text=f"• {issue} detected in {loc}", bg=CLR_BG).pack(side="left")
            tk.Button(f, text=action, command=lambda a=action: self.execute_repair(a), bg=CLR_ACC, fg="white", width=20).pack(side="right")

        tk.Button(self.container, text="🚀 MASTER OPTIMIZATION (Run All Repairs)", 
    ...
    def execute_repair(self, repair_type):
        ...
        lbl = tk.Label(rep_win, text="Initializing Engine...", fg="#00FF41", bg=CLR_CON, font=("Consolas", 10), wraplength=450)
        lbl.pack(pady=40)
        ...
        def repair_logic():
            try:
                ...
                if repair_type in ["Cleanup Ghost Rows", "ALL"]:
                    lbl.config(text="Status: Resetting UsedRange & Deleting Ghost Rows...")
                    for sh in wb.Sheets:
                ...
                reduction = ((self.results['size'] - new_size) / self.results['size']) * 100
                
                wb.Close()
                excel.Quit()
                
                msg = f"✅ SUCCESS!\nOriginal Size: {self.results['size']:.2f} MB\nNew Size: {new_size:.2f} MB\n"
                msg += f"Reduction: {reduction:.1f}% space saved."
                lbl.config(text=msg, fg="#00FF41")
