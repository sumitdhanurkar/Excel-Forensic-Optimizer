...
class ForensicProV108:
                ...
                while attempt < 3:
                    try:
                        ...
                        if "password" in str(e).lower():
                            # Ask user for password via UI
                            password = simpledialog.askstring("Encrypted File", f"File '{fname}' is password protected.\nPlease enter password:", show='*')
                            if not password: break # User cancelled
                            attempt += 1
                        else:
                            break

                if not wb:
                    self.batch_results.append({"name": fname, "size": "N/A", "dims": "LOCKED", "issues": ["Authentication Failed: Password missing or incorrect."], "health": "Critical"})
                    continue
                ...
                try:
                    ...
                    for sh in wb.Sheets:
                        ...
                        if last_cell.Row > 5000 and r < (last_cell.Row * 0.5):
                            issues.append(f"Phantom Data ({sh.Name}): {last_cell.Row} rows detected vs {r} active.")
                    ...
                    self.batch_results.append({
                        ...
                        "dims": f"[{total_rows}R x {total_cols}C]", 
                        "issues": issues,
    ...
    def display_final_audit(self):
        ...
        tk.Label(metrics_f, text=f"CPU: {cpu}%", font=("Segoe UI", 10, "bold"), bg="white", fg=CLR_HEADER).pack(side="left", padx=(0, 20))
        ...
        tk.Label(metrics_f, text=f"DISK: {disk}% Used", font=("Segoe UI", 10, "bold"), bg="white", fg=CLR_HEADER).pack(side="left", padx=20)
        ...
        for res in self.batch_results:
            ...
            # File Name + Row/Col Count
            tk.Label(card, text=f"📄 {res['name']}  {res['dims']}", font=("Segoe UI", 12, "bold"), bg="white", fg=CLR_HEADER).pack(anchor="w")
            ...
            if res['health'] == "Fully Compliant":
                ...
                compliance_info = [
                    "Structural Audit: UsedRange aligns with data; no phantom memory bloat.",
                    "Calculation Audit: Static references only; no volatile function lag.",
                    "Security Audit: Metadata is clean; no hidden VBA or unauthorized macros."
                ]
                for info in compliance_info:
                    tk.Label(card, text=f"• {info}", font=("Segoe UI", 9), fg="#444", bg="white", wraplength=900, justify="left").pack(anchor="w", padx=10)
            else:
                ...
                for prob in res['issues']:
                    ...
                    tk.Label(p_frame, text=f"DETECTED: {prob}", font=("Segoe UI", 9, "bold"), fg=CLR_ERR, bg="#FFF9F9", wraplength=900, justify="left").pack(anchor="w")
                    tk.Label(p_frame, text="SOLUTION: Optimize UsedRange / Replace volatile formulas / Check file integrity.", font=("Segoe UI", 8), fg="#444", bg="#FFF9F9").pack(anchor="w", padx=15)
